...
static int help(const char *name, const char *msg, int code)
{
    ...
    fprintf(code ? stderr : stdout,
            ...
            "  was successful. If the threshold was not reached, we return 1. In this mode\n"
            "  we process all armoured data even if we could end early.\n"
            "\n"
            "EXAMPLES\n"
            "  In this basic example, we split a series of detached PGP signatures,\n"
            "  passing each signature to the gpg command.\n"
            "\n"
            "\t~$ cat original_file.asc | xarmour -- gpg --verify - original_file\n"
            "\n"
            "AUTHOR\n"
